...
func QueueNextEndpoint(svc services.TaskService, stateView view.TaskStateView, payloadView view.TaskPayloadView) endpoint.Endpoint {
	return func(_ context.Context, request interface{}) (interface{}, error) {
		...
		next, err := stateView.Next(req.Queue)
		if err != nil {
			return TaskStateResponse{nil, "", "", err.Error()}, nil
		}
		if next == nil {
		...
		payload, err := payloadView.PayloadFor(next.TaskID)
		if err != nil {
			return TaskStateResponse{nil, "", "", err.Error()}, nil
		}

		return TaskStateResponse{next, payload, "", ""}, nil
	}
}
